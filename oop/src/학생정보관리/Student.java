...
public class Student {
	...
	// 필드 입력  소중한 학생정보이므로 접근제한 private
		...
		private int scoreAndroid;
		 ...
		 // 객체를 생성하는 순간에 실행되는 매서드 = 메인의 stu2 객체 형성
		 // --> alt shift s 단축키로 생성가능 --> 제네래이트 constructor using fields 버튼	
		// 필드에 있는 데이터를 객체(stu2)를 생성하는 순간에 초기화(초기값)를 시켜줄 수 있도록 로직 작성
	...
	// 매개변수가 아무것도 없는 생성자 즉 기본생성자 라고 부르는데 생략 가능하다 (디폴트 constructor)
	// 단 새로운 나만의 생성자(위의코드)를 만들게 되면 기본 생성자는 덮어씌워진다(사라진다) 그래서 다시 생성해줘야 한다.
	// 생성자도 오버로딩 (중복정의) 가능하다
	// 오버로딩: 매서드의 이름과 리턴타입이 동일한 상태에서 매개변수의 개수와 타입을 다르게 중복으로 정의하는 매서드 정의 기법
		
      // 중복으로 생성자 만듬(오버로딩) // 다트에서는 동일표현 public Student();가능 {}생략가능 
