...
<body>
	<!-- 1. 스크립트릿 : jsp 내부에서 java 언어를 로직으로 사용하기 위한 영역! -> 변수 선언과 연산등의 로직 구현 -->
	<!-- 1. 스크립트릿 : 자바언어를 로직하거나 처리하는 공간이지 변수를 불러오거나 매서드를 호출하는 공간은 아니다  -->
	<!-- 1. 스크립트릿 : 매서드 만들면 에러발생  -->
	<%
	int num1 = 25;
	int num2 = 5;
	int sum = num1 + num2;
	...
	<!-- 2. 표현식 : java 변수자체 혹은 연산된 로직등 혹은 매서드를 화면에 출력하기 위하여 사용되는 영역! -->
	...
	<!--3.선언문: 매소드를 선언하기 위해서 사용하는 영역 
	->전역매서드로 만들어진다!! **변수선언도 가능->전역변수화
	-->
	<!--선언문은 java로 변환시 해당 내용들을 전역으로 쓰도록 가장 맨위에 배치한다! 
	     하지만 여기서는 아무곳이든 !만 붙이면 그 자체가 전역이다-->
...
<!--.jsp 파일은 .java 와 .class 파일 확장자로 구성됨 -> html태그안에서 java코딩을 하는 것으로 자바파일을 class로 컴파일 한 후 html코드로 변환한다 -->
